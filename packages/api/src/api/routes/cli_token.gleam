...
pub fn handle_request(req: glen.Request) -> Promise(glen.Response) {
	case req.method {
		...
		_ ->
			promise.resolve(glen.text("Method not allowed", status.method_not_allowed))
	}
}
...
fn patch(req: glen.Request) -> Promise(glen.Response) {
	let res = {
		use token <- promise.try_await(promise.resolve(
			...
			|> result.replace_error(#("not_logged_in", status.ok)),
		))
		...
		let db_res =
			db.get_db()
			|> db.execute(
				"UPDATE users SET \"cli_token\" = hex(randomblob(24)) WHERE users.id = '"
		...
		use db_res <- promise.try_await(
			db_res
			|> utils.promise_error(#("failed_to_update", status.internal_server_error)),
		)
		let db_res =
			db_res
			|> db.as_user
