	...
	let effect =
		effect.batch([
			...
			effect.from(fn(dispatch) {
				auth.create_auth_listener(fn(user, _) {
	...
	case msg {
		OnUserChanged(user) -> {
			case user {
				...
				auth.NoUser -> model.Model(..model, user:, user_data: auth.UserDataNoUser) |> route_guard([])
				auth.User(..) -> model.Model(..model, user:, user_data: auth.UserDataNotLoaded) |> route_guard([get_user_data(on_response: OnApiReturnedUserData, token: user.session_token)])
			}
		}
