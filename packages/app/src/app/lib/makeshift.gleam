...
pub type Element = element.Element(Nil)
pub type Attribute = attribute.Attribute(Nil)
pub type Attributes = List(Attribute)
pub type Children = List(Element)
...
pub fn set_status(ctx: RouteContext, status: Int) {
	let new_res = response.new(status)
...
fn bulk_set_header(res: wisp.Response, headers: List(#(String, String))) -> wisp.Response {
	case headers {
		[] -> res
		[#(key, value), ..rest] -> bulk_set_header(response.set_header(res, key, value), rest)
	}
}